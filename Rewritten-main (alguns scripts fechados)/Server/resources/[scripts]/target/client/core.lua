		...
		options = {
			{
				...
				event = "dynamic:Experiences",
				label = "Verificar Experiências",
				tunnel = "client"
			}
		}
	})
		...
		if Hitable == 1 then
			...
			for Index,v in pairs(Zones) do
				if #(Coords - Zones[Index]["center"]) <= 10 then
					...
					DrawSprite("Targets","Point",0,0,0.02,0.02 * GetAspectRatio(false),0,255,255,255,255)
					ClearDrawOrigin()
				end
				...
				if Zones[Index]:isPointInside(HitCoords) and #(Coords - Zones[Index]["center"]) <= v["targetoptions"]["Distance"] then
					...
					while Sucess do
						...
						SetDrawOrigin(Zones[Index]["center"]["x"],Zones[Index]["center"]["y"],Zones[Index]["center"]["z"])
						DrawSprite("Targets","Pointed",0,0,0.02,0.02 * GetAspectRatio(false),0,255,255,255,255)
						ClearDrawOrigin()
