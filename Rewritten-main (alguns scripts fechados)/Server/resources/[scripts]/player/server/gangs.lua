	...
	if Passport then
		if vRP.HasPermission(Passport, Gang) then
			if vRP.Banks("See", false, Gang) > 0 then
				...
				TriggerClientEvent("player:GangsVerifyPanel", source, Gang, parseFormat(vRP.Banks("See", false, Gang)))
			else
			...
			if vRP.HasPermission(Passport, Gang, 1) then
				if vRP.Banks("See", false, Gang) > 0 then
					if (not GangsBank[Passport] or os.time() > GangsBank[Passport]) then
						...
						if vRP.Request(source, "Cofre dos "..Gang, "Você deseja realmente retirar <b>$"..parseFormat(vRP.Banks("See", false, Gang)).." "..ItemName(DefaultDollars1).."</b> do <b>Cofre dos "..Gang.."</b>?") then
							if GangsLog then
