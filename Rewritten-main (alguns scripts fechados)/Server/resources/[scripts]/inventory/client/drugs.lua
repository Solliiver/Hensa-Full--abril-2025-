...
CreateThread(function()
	while true do
		...
		if not IsPedInAnyVehicle(Ped) and LocalPlayer["state"]["Drugs"] and not LocalPlayer["state"]["SafeZone"] then
			local Progress = 1000
			...
			if Entitys and not Entity(Entitys)["state"]["Drugs"] then
				...
				SetDrawOrigin(EntityCoords)
				DrawSprite("Targets","E",0,0,0.02,0.02 * GetAspectRatio(false),0,255,255,255,255)
				ClearDrawOrigin()
