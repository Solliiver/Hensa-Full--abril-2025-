		...
		if not IsPedInAnyVehicle(Ped) and Drops[Route] then
			...
			for _,v in pairs(Drops[Route]) do
				if #(Coords - v["Coords"]) <= 50 then
					...
					DrawSprite("Targets","Drop",0,0,0.02,0.02 * GetAspectRatio(false),0,255,255,255,255)
					ClearDrawOrigin()
				end
			end
		end
...
local Atm = {
	...
	["145"] = vec3(1171.5, 2702.99, 38.21),
	["146"] = vec3(1172.43, 2702.99, 38.22)
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- ATM
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			options = {
				{
					...
					event = "player:GetScratchCard",
					label = "Comprar Raspadinha",
					tunnel = "server"
				}
			}
		})
