...
function Hensa.Inventory()
	...
	if Passport then
		...
		for Index,v in pairs(Inv) do
			if (parseInt(v["amount"]) <= 0 or not ItemIndex(v["item"])) then
	...
	if Passport and not Active[Passport] then
		...
		elseif Work == "Ballas" then
			...
			if vRP.TakeItem(Passport,BallasItem,1,true,Slot) then
				vRP.Banks("Add", ItemPrice, "Ballas")
		...
		elseif Work == "Families" then
			...
			if vRP.TakeItem(Passport,FamiliesItem,1,true,Slot) then
				vRP.Banks("Add", ItemPrice, "Families")
				TriggerClientEvent("Notify", source, "verde", "Adicionado <b>$"..parseFormat(ItemPrice).."</b> ao cofre dos <b>"..Work.."</b>.", "Sucesso", 5000)
		...
		elseif Work == "Aztecas" then
			...
			if vRP.TakeItem(Passport,AztecasItem,1,true,Slot) then
				vRP.Banks("Add", ItemPrice, "Aztecas")
				TriggerClientEvent("Notify", source, "verde", "Adicionado <b>$"..parseFormat(ItemPrice).."</b> ao cofre dos <b>"..Work.."</b>.", "Sucesso", 5000)
		...
		elseif Work == "Bloods" then
			...
			if vRP.TakeItem(Passport,BloodsItem,1,true,Slot) then
				vRP.Banks("Add", ItemPrice, "Bloods")
				TriggerClientEvent("Notify", source, "verde", "Adicionado <b>$"..parseFormat(ItemPrice).."</b> ao cofre dos <b>"..Work.."</b>.", "Sucesso", 5000)
	...
	if Passport and not Active[Passport] then
		...
		elseif ItemType(Full) == "Munição" then
			...
			if Check then
				...
				if Item ~= WeaponAmmo(Weapon) or Amount <= 0 then
					return
				end
				...
				repeat
					if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
						...
						if Service == "Lixeiro" then
							itemSelect = { "trashbag", math.random(2, 4) }
						...
						elseif Service == "Electric" then
							...
							SetTimeout(10000, function()
								TriggerClientEvent("vRP:Explosion", source, Coords)
								exports["inventory"]:DropInCoords(Passport, source, "metalwire", math.random(3, 6), Coords)
							end)
...
-- INVENTORY:HOOKAH
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("inventory:Hookah")
AddEventHandler("inventory:Hookah",function()
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
		if vRP.Request(source, "Hookah and Bar", "Deseja fumar por <b>$100 Dólares</b>?") then
			if vRP.PaymentFull(Passport, 100) then
				...
				repeat
					if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
						...
						TriggerClientEvent("Notify",source,"verde","Você ficou <b>100% sem estresse</b>.","Sucesso",5000)

						local Random = math.random(100)
						if Random >= 50 then
							if not vRP.Medicplan(source) then
								vRP.UpgradeCough(Passport,20)
								TriggerClientEvent("Notify",source,"amarelo","Seu pulmão não aguentou e você ficou doente.","Atenção",5000)
							end
						end
...
-- INVENTORY:TRASHBAGS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("inventory:TrashBags")
AddEventHandler("inventory:TrashBags",function()
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
		local TrashItem = "trashbag"

		if vRP.ConsultItem(Passport, TrashItem, 1) then
			vRPC.AnimActive(source)
			...
			TriggerClientEvent("inventory:Close", source)
			TriggerClientEvent("Progress", source, "Entregando saco", 4000)
			vRPC.CreateObjects(source, "mp_safehouselost@", "package_dropoff", "prop_ld_rub_binbag_01", 16, 28422, 0.0, -0.05, 0.05, 180.0, 0.0, 0.0)

			repeat
				if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
					Active[Passport] = nil

					if vRP.TakeItem(Passport, TrashItem, 1, true) then
						vRP.UpgradeStress(Passport, 1)

						local Rand = math.random(#TrashList)
						local Amount = math.random(TrashList[Rand]["Min"], TrashList[Rand]["Max"])
						vRP.GenerateItem(Passport, TrashList[Rand]["Item"], Amount, true)
					end
			...
			TriggerClientEvent("Notify", source, "amarelo", "Você precisa de <b>1x "..ItemName(TrashItem).."</b>.", "Atenção", 5000)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
