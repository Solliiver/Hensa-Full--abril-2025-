	...
	if Passport then
		for Item,v in pairs(List) do
			...
			if vRP.ConsultItem(Passport,Item,Amount) then
				...
				TriggerClientEvent("Notify", source, "amarelo", "Você não possúi nada para oferecer.", "Atenção", 5000)
		...
		for k,v in pairs(DrugsInfluences) do
			...
			if Distance <= v[4] then
				...
				if math.random(100) >= 50 then
					vRP.GenerateItem(Passport,"vote",1,true)
				end
			end
		end
