...
Use = {
	...
	["woodenbarrel"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		if vRP.TakeItem(Passport,Full,1,true,Slot) then
			...
			if Danger >= 90 and not vRP.Medicplan(source) then
		...
		repeat
			if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
				...
				if vRP.TakeItem(Passport,Full,1,true,Slot) then
					...
					if RandomCough >= 50 and not vRP.Medicplan(source) then
	...
	["enginea"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRPC.InsideVehicle(source) then
			local Vehicle, Network, Plate, vehName = vRPC.VehicleList(source, 2)
			if Vehicle then
	...
	["drift"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRP.InsideVehicle(source) then
			local Vehicle, Network, Plate, Model, Class = vRPC.VehicleList(source, 2)
