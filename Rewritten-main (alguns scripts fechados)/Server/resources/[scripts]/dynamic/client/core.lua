...
-- DYNAMIC:EXPERIENCES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("dynamic:Experiences")
AddEventHandler("dynamic:Experiences", function()
	if not Dynamic then
		local Experience = vSERVER.Experience()
		for Name,Exp in pairs(Experience) do
			exports["dynamic"]:AddButton(Name, "Você possuí <blue>"..Exp.." pontos</blue> no nível <blue>"..ClassCategory(Exp).."</blue>.", "", "", false, false)
		end
...
-- DYNAMIC:PEDSTATS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("dynamic:PedStats")
AddEventHandler("dynamic:PedStats", function()
	if not Dynamic then
		local Stats = vSERVER.PedStats()
		for Name,Points in pairs(Stats) do
			exports["dynamic"]:AddButton(Name, "Você possuí um total de <blue>"..Points.." "..Name.."</blue>.", "", "", false, false)
		end
...
RegisterCommand("AdminFunctions", function()
	if LocalPlayer["state"]["Admin"] and not IsPauseMenuActive() then
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not Dynamic and not LocalPlayer["state"]["Bennys"] and MumbleIsConnected() then
			if not GlobalState["Commands"] then
